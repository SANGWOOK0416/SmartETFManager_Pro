...
본 프로젝트의 안정성과 성능을 검증하기 위해 핵심 기능 단위로 테스트 시나리오를 작성하고 실험을 수행하였습니다.

### 5.1. 상세 테스트 시나리오 (Test Scenarios)
| 테스트 ID | 테스트 항목 | 사전 조건 | 테스트 절차 (시나리오) | 예상 결과 | 실제 결과 |
|---|---|---|---|---|---|
| **TS-01** | **매수 트랜잭션 및<br>평단가 연산** | DB 연결 완료 | 1. VOO 10주 최초 매수<br>2. VOO 5주 추가 매수 진행 | 최초 및 추가 매수가를 가중 평균하여 정확한 평균 단가가 재계산되고 DB에 갱신됨 | 정상 (Pass) |
| **TS-02** | **전량 매도 및<br>데이터 무결성** | TS-01 통과 | 1. 포트폴리오에서 VOO 선택<br>2. 보유 수량과 동일한 15주 매도 | UI에서 해당 종목이 사라지며, DB에서 레코드가 완벽히 `DELETE` 됨 | 정상 (Pass) |
| **TS-03** | **API 캐싱(Caching)<br>성능 검증** | 캐시 모듈 구동 | 1. `/api/chart/QQQ` 1회차 호출<br>2. 5초 후 동일 엔드포인트 2회차 호출 | 1회차는 외부 API 통신(약 800ms), 2회차는 캐시 히트로 즉시 응답(약 15ms) | 정상 (Pass) |
| **TS-04** | **UX 반응성 및<br>예외 처리** | 앱 정상 구동 | 1. 존재하지 않는 티커 검색<br>2. 매수 수량에 음수(-5) 입력 시도 | 존재하지 않는 종목 차단 및 잘못된 수량 입력 시 Toast 경고 알림 발생 | 정상 (Pass) |
